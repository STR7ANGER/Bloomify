...
const sellerRegister = async (req, res) => {
  try {
    const { name, comapnyEmail, companyNumber, password, companyName, companyAddress, upi } = req.body;

    // Check if all required fields are provided
    if (!name || !comapnyEmail || !companyNumber || !password || !companyName || !companyAddress || !upi) {
    ...
    if (!companyAddress.street || !companyAddress.city || !companyAddress.state || !companyAddress.pincode) {
      return res.status(400).json({
        success: false,
...
export { sellerLogin, sellerRegister };